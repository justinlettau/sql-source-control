...
module.exports = {
     ...
     */
    webConfigFile: path.join(process.cwd(), 'Web.config'),
     ...
     */
    getWebConfigConn: function () {
        let parser = new xml2js.Parser();
        let webconfig;
        let conn;

        if (!fs.existsSync(this.webConfigFile)) {
            // no web config present
            return;
        }

        // read config file
        webconfig = fs.readFileSync(this.webConfigFile, 'utf-8');
