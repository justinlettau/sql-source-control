            ...
            .then((data) => {
                ...
                for (let row of data) {
                    ...
                    if (config.output.idempotency) {
                        let prefix = getIdempotency(row.schema, row.name, row.type);
 ...
 */
function getIdempotency(schema, name, type) {
    const prefix = `if exists (select * from sys.objects where object_id = object_id('${schema}.${name}') and type = '${type}')
drop procedure ${name}
go
