...
module.exports = () => {
    ...
    // connect to db
    const conn = new sql.Connection(config.connection, (err) => {
        if (err) {
        ...
        Promise.all([
            ...
            new sql.Request(conn).query(sys.foreignKeys),
            ...
            for (let item of tables) {
                ...
                const content = script.table(item, columns, primaryKeys, foreignKeys, indexes);
