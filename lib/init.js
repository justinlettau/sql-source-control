...
module.exports = function (options) {
    let webConfigConn = util.getWebConfigConn(options.webconfig);
    let defaultConn = {
        server: '',
        port: 1433,
        database: '',
    ...
    if (options.skip) {
        let conn = webConfigConn ? util.webConfigFile : defaultConn;
        ...
        inquirer.prompt([
            ...
            let conn = answers.path ? util.webConfigFile : answers;
            delete answers.path;
            createFile({ connection: conn });
        });
    }
...
function createFile(config) {
    ...
    // save file
    fs.outputFile(util.configFile, content, function (error) {
        if (error) {
